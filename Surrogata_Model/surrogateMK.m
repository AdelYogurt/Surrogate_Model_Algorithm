...
% y=zeros(1,101);
% for x_index=1:101
%     y_MK(x_index)=predict_function_MK(x(x_index));
%     y(x_index)=predict_function(x(x_index));
% end
% line(x,benchmark.singleForresterObject(x),'Color','r','LineStyle','-')
% line(x,benchmark.singleForresterObjectLow(x),'Color','r','LineStyle','--');
% line(x,y_MK,'Color','b','LineStyle','-')
% line(x,y,'Color','b','LineStyle',':');
% legend({'singleForresterObject','singleForresterObjectLow','MK','K'})

function [predict_function,MK_model]=interpMultiKrigingPreModel...
    (XHF, YHF, XLF, YLF)
% construct Multi-Level Kriging version 1
