...
SRSM_value_data=zeros(1,repeat_time);
SRSM_NFE_data=zeros(1,repeat_time);
...
for repeat_index=1:repeat_time
    ...
    [SRS_x,SRS_fval,SRS_NFE,SRS_output]=optimalSurrogateKriging...
    ...
    delete(data_library_name);
    delete('result_total.txt');
    [SRSM_x,SRSM_fval,SRSM_NFE,output]=optimalSurrogateKrigingM...
        (object_function,variable_number,low_bou,up_bou,nonlcon_function,...
        cheapcon_function,model_function);
    SRSM_value_data(repeat_index)=SRSM_fval;
    SRSM_NFE_data(repeat_index)=SRSM_NFE;
end
% average
SRS_value_average=mean(SRS_value_data);
SRS_NFE_average=mean(SRS_NFE_data);
SRSM_value_average=mean(SRSM_value_data);
SRSM_NFE_average=mean(SRSM_NFE_data);

% median
SRS_value_median=median(SRS_value_data);
SRS_NFE_median=median(SRS_NFE_data);
SRSM_value_median=median(SRSM_value_data);
SRSM_NFE_median=median(SRSM_NFE_data);

% variance
SRS_value_var=var(SRS_value_data);
SRS_NFE_var=var(SRS_NFE_data);
SRSM_value_var=var(SRSM_value_data);
SRSM_NFE_var=var(SRSM_NFE_data);
...
SRSM_value_min=min(SRSM_value_data);
SRSM_NFE_min=min(SRSM_NFE_data);
SRSM_value_max=max(SRSM_value_data);
SRSM_NFE_max=max(SRSM_NFE_data);

% boxplot
box_value_data=[SRS_value_data' SRSM_value_data'];
box_NFE_data=[SRS_NFE_data' SRSM_NFE_data'];
g_SRS = repmat({'SRS'},repeat_time,1);
g_SRSM = repmat({'SRSM'},repeat_time,1);
label=[g_SRS;g_SRSM];
